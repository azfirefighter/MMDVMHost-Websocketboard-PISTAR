...
otherwise, you're going to have to do a little editing in one of the python libraries to convert celcius to farenheit
    ...
    4. Edit the _pslinux.py file (nano _pslinux.py)
           i. Scroll down 1200 lines or so until you reach a section that shows "# --- sensors"
           ii. Look for the line that says "current = (float(cat(path)) / 1000.0"  This should be around line 1210 or so
           iii. Change it to read "current1 = (float(cat(path)) / 1000.0" and press enter
           iv. USE SPACES to move the cursor until it's under the "c" from the line above
           v. Add "current = float(current1 * 1.8 + 32)
