...
DMRIDPATH=/opt/MMDVMDash
DMRIDFILE=${DMRIDPATH}/DMRIds.dat

# DMR IDs now served by RadioID.net
DATABASEURL='https://radioid.net/static/user.csv'
...
if [ "$(id -u)" != "1001" ] 
then
	echo "This script must be run as mmdvm" 1>&2
	exit 1
fi
...
# Generate new file
wget ${DATABASEURL} -O ${DMRIDFILE}
cat ${DMRIDFILE} | sed -e 's/\t//g' | awk -F"," '/,/{gsub(/ /, "", $2); printf "%s\t%s\t%s\n", $1, $2, $3}' | sed -e 's/\(.\) .*/\1/g' > ${DMRIDPATH}/DMRIds.tmp
